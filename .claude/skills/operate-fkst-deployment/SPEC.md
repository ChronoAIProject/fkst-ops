...
| A4 | Keep as doctrine; diagnostic prior, not mechanically verifiable | Operating doctrine | 无 | Change the external exit-code convention |
...
| A9 (first) | Keep as highest-value doctrine; remote runtime fact | Operated remote | 无 | Change the remote prerequisite |
| A9 (second) | Test rejected; negative source grep is refactor-defeated | Root `SPEC.md` | 无 | Add branch creation behind any spelling |
| A10 | Keep; unverifiable by construction from the same observation | Operating doctrine | 无 | Make the observation sufficient |
...
| A12 | Delete local restatement; reference owner only | `fkst-deployments` authoritative policy | 无 | Change or relocate the owner policy |
