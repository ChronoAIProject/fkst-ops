...
`fkst-ops` is the version-controlled, repository-agnostic operational mechanism for FKST deployments. A deployment repository pins `fkst-ops`, supplies a declaration, and invokes it through a repository-owned bootstrap. The dependency direction is strictly `deployment repository -> pinned fkst-ops`.
...
## 7. Edge-Owned Bootstrap

There is one bootstrap per **deployment repository**, not per deployment. Before `fkst-ops` is hydrated, only the deployment can read its own lock; the bootstrap is therefore an edge-owned, irreducible trust root. It is schema-agnostic and performs exactly three duties, in order:

1. locate its repository-owned lock;
2. hydrate pinned `fkst-ops` and verify its full `resolved.rev` and canonical `tree_sha256`;
3. hand pinned `fkst-ops` the declaration path plus machine-reference resolution inputs.

Generation 1 reuses the working `fkst-website/scripts/run.sh` shape, changing only the external-source identifier and delegation target to `fkst-ops`. No bootstrap framework is introduced. The current script reads the lock, clones with `--no-checkout`, checks out the revision, and delegates at `fkst-website/scripts/run.sh:15-51,54-79,107-123`.

The ordered handover is one total fail-closed declaration preflight spanning the bootstrap and pinned `fkst-ops`. The bootstrap verifies only the pin and tree in duty 2 and never parses, validates, or understands the declaration schema or any provider contract. After duty 3, pinned `fkst-ops` resolves machine references and exclusively validates the resolved schema, provider contracts, checkout roots, package roots, and provider executable entry points. Outputs created by providers, including the engine binary build path, are action preconditions rather than declaration-preflight pre-existence requirements. Nothing in deployment operational state mutates until pinned `fkst-ops` completes all declaration validation successfully.
...
No other design decision is open. Ownership direction, first adopter, bootstrap location, no-dual-mode rule, optional-profile boundary, deletion set, and `fkst-hostctl` disposition are fixed.
