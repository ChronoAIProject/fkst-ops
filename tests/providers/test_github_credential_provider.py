...
def test_github_cli_user_peer_lock_wait_is_bounded() -> None:
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        runtime_root = root / "runtime"
        attestation_directory = runtime_root / PROVIDER_MODULE.ATTESTATION_DIRECTORY_NAME
        attestation_directory.mkdir(parents=True)
        lock_path = attestation_directory / PROVIDER_MODULE.ATTESTATION_LOCK_NAME
        gh, calls = make_github_cli_user_gh(root)
        ...
        assert peer.stdout.readline() == "locked\n"

        started = time.monotonic()
        try:
            result = invoke(
                root / "missing-app-resolver", gh, "declared-user",
                source="github-cli-user", FKST_RUNTIME_ROOT=str(runtime_root),
            )
        ...
        assert result.returncode == 0, result.stderr
        assert elapsed < 1.0, f"peer lock delayed verification for {elapsed:.3f}s"
        assert call_counts(calls) == (1, 1, 1)
        assert list(runtime_root.rglob(PROVIDER_MODULE.ATTESTATION_STATE_NAME)) == []
